...
[What else is on BioC for targeted/etc.]
...
The raw example sequencing data used in this vignette can be obtained from the github repo `jmf47/MDTSdata` using:
```{r, eval=FALSE}
...
2. A bw file that includes the 100mer mappability of the genome
3. A pedigree file 
...
This portion of the vignette requires using raw data from `MDTSData`. However the resulting bins are included in the `data` directory of MDTS as a `.RData`.
