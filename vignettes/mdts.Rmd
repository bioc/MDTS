...
Slighly processed versions of the data is included with `MDTS`. To obtain the 
raw data use:
...
*med* parameter. The choice of *med* is a tradeoff between sensitivity and false
positives. A larger *med* decreases sensitivity and false positives. For our 
publication, we used *med=160*, and for this example we used *med=100*.
...
This portion of the vignette requires using raw data from `MDTSData`. However 
...
# min is the minimum read depth for a location to be included in a proto region
# med is the median number of reads across the n samples in a bin.
bins = calcBins(metaData=pD, n=5, readLength=100, medianCoverage=150, 
